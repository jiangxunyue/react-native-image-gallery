...
export default class ViewTransformer extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.minScale !== this.props.minScale) {
            if (this.state.scale < nextProps.minScale) {
                this.setState({
                    scale: nextProps.minScale,
                });
            }
        }
    }
    ...
    performDoubleTapUp(pivotX, pivotY) {
        ...
        let scaleBy;
        if (curScale > (this.props.minScale + this.props.maxScale) / 2) {
    ...
    forceUpdateTransform(transform) {
        this.setState({...transform, scale: this.props.minScale});
    }

    normalizeTransform(transform) {
        const {scale, translateX, translateY} = transform;
        const newScale = typeof scale === 'undefined' ? this.state.scale : Math.round(scale);
        const newTranslateX = typeof translateX === 'undefined' ? this.state.translateX : Math.round(translateX * newScale) / newScale;
        const newTranslateY = typeof translateY === 'undefined' ? this.state.translateY : Math.round(translateY * newScale) / newScale;
        return {
            ...transform,
